...
public final class DataSourcePool {
    ...
    /**
     *ThreadLocal是线程局部变量，使用场景：当很多线程需要多次使用同一个对象，并且需要该对象具有相同初始化值的时候最适合使用ThreadLocal。
     * ThreadLocal的内部实现原理：
     ...
     * 每次线程在get的时候，都从自己的变量中取值，既然从自己的变量中取值，那肯定就不存在线程安全问题，
    ...
    public static <T> boolean insertEntity(Class<T> entityClass, Map<String, Object> fieldMap) {
        if (CollectionUtil.isEmpty(fieldMap)) {
            LOGGER.error("保存新增对象失败: 入参fieldMap为空");
            return false;
        }
    ...
    public static <T> boolean updateEntity(Class<T> entityClass, long id, Map<String, Object> fieldMap) {
        if (CollectionUtil.isEmpty(fieldMap)) {
            LOGGER.error("无法更新实体类: 入参fieldMap为空");
            return false;
        }
