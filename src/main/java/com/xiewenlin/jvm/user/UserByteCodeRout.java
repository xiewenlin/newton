 ...
 * @ClassName UserByteCodeRout
 * @Description 操作数例程
 * @Date 2019/3/22 15:31
 * @Version V1.0.0
 */
public enum UserByteCodeRout {
    /**
     * 将第0个引用类型局部变量推送至栈顶
     */
    ALOAD_0(Constants.ALOAD_0){
        @Override
        public void invoke(RunEnvironment env, StackFrame frame, byte[] operands) throws Exception {
    ...
    GETSTATIC(Constants.GETSTATIC){
        ...
        public void invoke(RunEnvironment env, StackFrame frame, byte[] operands) throws Exception {
            ...
            //静态字段所在的类
            ClassX clazz = env.getVm().getClass(info.getClassName());
            FieldX field = clazz.getField(info.getNameAndTypeInfo().getName());
            //静态字段的值
    ...
    INVOKESPECIAL(Constants.INVOKESPECIAL){
        ...
        public void invoke(RunEnvironment env, StackFrame frame, byte[] operands) throws Exception {
            ...
            ObjectX thiz = (ObjectX) argsArr[0];
    ...
    NEW(Constants.NEW){
        ...
        public void invoke(RunEnvironment env, StackFrame frame, byte[] operands) throws Exception {
            ...
            ClassX clazz = env.getVm().getClass(info.getName());
    ...
    private static Map<Short, UserByteCodeRout> codeMapping =  new HashMap<>();
    private final short code;

    UserByteCodeRout(short code){
        this.code = code;
    }

    public abstract void invoke(RunEnvironment env, StackFrame frame, byte[] operands) throws Exception;

    public short getCode() {
        return code;
    }

    public static UserByteCodeRout valueOf(short code){
        UserByteCodeRout op = codeMapping.get(code);
        if(op == null){
    ...
    static {
        for (UserByteCodeRout op: values()) {
